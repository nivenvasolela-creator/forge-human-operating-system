...
export function ReflectionScreen() {
    ...
    return (
      <div className="max-w-xl mx-auto py-12 px-6 space-y-10">
        <div className="space-y-1">
          <p className="text-xs font-mono text-muted-foreground uppercase tracking-[0.2em]">Reflection logged</p>
          <h1 className="text-2xl font-semibold text-foreground">Day complete.</h1>
        ...
        <div className="space-y-3">
          <button
            ...
            className="block text-sm font-mono text-primary hover:text-foreground transition-colors"
          >
            Start tomorrow &rarr;
          </button>
          <button
            ...
            <div className="space-y-6">
              {reflections.slice(0, 5).map((r) => (
                <div key={r.id} className="space-y-3">
                  <p className="text-xs font-mono text-muted-foreground">{r.date}</p>
  ...
  return (
    <div className="max-w-xl mx-auto py-12 px-6 space-y-12">
      ...
      {/* Header */}
      <div className="space-y-1">
        <p className="text-xs font-mono text-muted-foreground uppercase tracking-[0.2em]">Reflection</p>
        <h1 className="text-2xl font-semibold text-foreground">End of day.</h1>
        <p className="text-sm text-muted-foreground leading-relaxed">
      ...
      <div className="space-y-8">
        <Question
          ...
          value={did}
          onChange={setDid}
          placeholder="I finished the product spec, worked out, and read 20 pages..."
          required
        />
        <Question
          number="02"
          ...
          value={blocked}
          onChange={setBlocked}
          placeholder="I got distracted by email for 2 hours. Didn't protect my morning block..."
        />
        <Question
          number="03"
          ...
          placeholder="Phone goes in another room before 10am. Only 3 tabs open. No meetings before noon..."
        />
      </div>
...
function ReflectionRow({ label, content }: { label: string; content: string }) {
  return (
    <div className="flex gap-3">
      <span className="text-xs font-mono text-muted-foreground/40 shrink-0 mt-0.5 w-14">{label}</span>
      <p className="text-sm text-muted-foreground/70 leading-relaxed flex-1">{content}</p>
    </div>
  )
}
